...
/** Per-client shape so the five reports have visibly different ROAS values. */
const CLIENT_SEEDS = [
...
async function seed() {
  ...
  if (missing > 0) {
    logger.warn(
      `${missing} of 5 SLACK_WEBHOOK_* values are unset in .env — those clients will seed with an empty URL. ` +
        'That is fine while USE_MOCK_SLACK=true.',
    );
  }
  ...
  logger.info('cleared clients, daily_stats and notification_logs');
  ...
  logger.info(`inserted ${CLIENT_SEEDS.length} clients`);
  ...
  for (const client of CLIENT_SEEDS) {
    for (let daysAgo = 0; daysAgo < 3; daysAgo += 1) {
      ...
      for (const channel of CHANNELS) {
        ...
        stats.push({
          client_id: client.id,
          // Spread entries through the day so a naive whole-day assumption shows up.
  ...
  logger.info(`inserted ${stats.length} daily_stats rows (${CLIENT_SEEDS.length} clients × 3 days × ${CHANNELS.length} channels)`);
